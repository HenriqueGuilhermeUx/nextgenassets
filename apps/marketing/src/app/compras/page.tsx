...
export default function ComprasPage() {
  ...
  async function load() {
    const res = await fetch(`${API}/smart-operations/purchases?partnerSlug=${encodeURIComponent(partnerSlug)}`);
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-white">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-6 rounded-[2rem] border border-white/10 bg-white/10 p-8 md:p-10">
          <div className="text-sm font-black uppercase tracking-[0.25em] text-emerald-300">Compras</div>
          <h1 className="mt-4 text-4xl font-black md:text-6xl">Nota aprovada vira compra operacional.</h1>
          <p className="mt-4 max-w-4xl text-white/65">Fornecedores, itens, impostos, total e vencimentos extraídos da nota. A compra só nasce depois da confirmação humana.</p>
        ...
        <section className="mt-8 space-y-4">
          {rows.length === 0 && <div className="rounded-3xl bg-white/10 p-6 text-white/60">Nenhuma compra registrada ainda.</div>}
          {rows.map((row) => (
            <div key={row.id} className="rounded-3xl border border-white/10 bg-white/10 p-6">
              <div className="flex flex-wrap justify-between gap-3">
                <div>
                  <h2 className="text-2xl font-black text-emerald-300">{row.supplierName || 'Fornecedor não identificado'}</h2>
                  <p className="mt-1 text-sm text-white/50">Nota {row.invoiceNumber || '-'} · {row.invoiceDate || 'sem data'}</p>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-black">{money(row.totalAmount)}</div>
