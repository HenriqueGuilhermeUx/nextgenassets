...
export default function AdminHome() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/partners').then(setPartners).catch(() => setApiOnline(false)),
      api.get('/executions?limit=10').then(setExecutions).catch(() => {}),
      api.get('/executions/stats/summary').then(setStats).catch(() => {})
        ...
        {!apiOnline && (
          <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl">
            ...
            <div className="text-sm text-red-700 mt-1">
              Verifique se o backend tá no ar: <a href="https://nga-api-yr9w.onrender.com/health" target="_blank" className="underline">/health</a>
            </div>
          </div>
        )}
                ...
                <tbody>
                  {executions.map(e => (
                    <tr key={e.id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-500">{e.createdAt ? new Date(e.createdAt).toLocaleString('pt-BR') : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function Sidebar({ apiOnline }: { apiOnline: boolean }) {
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-gray-900 text-white p-6 overflow-y-auto">
      ...
      <div className="mt-8 p-3 bg-gray-800 rounded-lg text-xs text-gray-400">
        ...
        <div className="mt-2 flex items-center gap-1.5">
          <span className={`w-1.5 h-1.5 rounded-full ${apiOnline ? 'bg-green-500' : 'bg-red-500'}`}></span>
